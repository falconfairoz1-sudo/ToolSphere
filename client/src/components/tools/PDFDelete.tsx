...
export default function PDFDelete() {
  ...
  const deletePages = async () => {
    if (!file || !pageRange) {
      alert('Please select a file and specify pages to delete');
      return;
    }
    ...
    try {
      ...
      if (pagesToDelete.length >= totalPages) {
        alert('Cannot delete all pages from PDF');
        setDeleting(false);
        return;
      }
      ...
      const pagesToKeep = allPages.filter(page => !pagesToDelete.includes(page));
      ...
      link.href = url;
      link.download = `deleted_pages_${file.name}`;
      link.click();
      
      URL.revokeObjectURL(url);
      alert(`Successfully deleted ${pagesToDelete.length} page(s)!`);
    } catch (error) {
      console.error('Error deleting pages:', error);
      alert('Failed to delete pages. Please check your page range.');
    } finally {
      setDeleting(false);
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 py-12 px-4">
      <div className="max-w-4xl mx-auto">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8">
          ...
          {file && (
            <div className="mb-6">
              ...
              <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">
                Enter page numbers to delete, separated by commas. Use hyphens for ranges (e.g., 1-3, 5, 7-9)
              </p>
            </div>
          )}
          ...
          <button
            onClick={deletePages}
            disabled={!file || !pageRange || deleting}
